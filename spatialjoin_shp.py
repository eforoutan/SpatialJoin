...
def spatial_join(input_shapefile, join_shapefile):
    try:
        ...
        output_shapefile = os.path.join(os.getcwd(), "spatial_joined_shapefile.shp")
        output_geojson = os.path.join(os.getcwd(), "spatial_joined_shapefile.geojson")
        output_csv = os.path.join(os.getcwd(), "spatial_joined_csv.csv")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python spatial_join.py <input_shapefile> <join_shapefile>")
        sys.exit(1)

    input_shapefile = sys.argv[1]
    join_shapefile = sys.argv[2]

    output_shapefile, output_geojson, output_csv = spatial_join(input_shapefile, join_shapefile)
